...
$global:HomelabWindowsScriptRoot = $PSScriptRoot
...
function Test-WslDistroInstalled {
    ...
    $list = Invoke-WslCli -Arguments @('--list', '--quiet')
    if ($list.Corrupted -or $list.ExitCode -ne 0) {
        return $false
    }
    $distros = $list.Output | ForEach-Object { ("$_" -replace '\x00', '').Trim() } | Where-Object { $_ -ne '' }
...
function Get-AnsibleSessionUserProfile {
    param([System.Management.Automation.PSCredential]$Credential)
    $result = Invoke-AsUser -Credential $Credential -FilePath 'cmd.exe' -ArgumentList @('/c', 'echo', '%USERPROFILE%')
    if ($result.ExitCode -ne 0) { return $null }
    return ($result.OutputText -replace [char]0, '').Trim()
}
...
function Export-AnsibleLxssJsonFromSession {
    ...
    $sourceScript = Join-Path $global:HomelabWindowsScriptRoot 'lib\export-ansible-lxss.ps1'
    if (-not (Test-Path -LiteralPath $sourceScript)) {
        Write-Warn "export-ansible-lxss.ps1 not found at $sourceScript"
        return $null
    }

    # Copy to ProgramData so ansible can execute it (repo may live under admin-only OneDrive).
    $exportScript = Join-Path $global:BootstrapSharedTemp 'export-ansible-lxss.ps1'
    Copy-Item -LiteralPath $sourceScript -Destination $exportScript -Force

    $jsonPath = Join-Path $global:BootstrapSharedTemp 'ansible-lxss.json'
    Remove-Item -LiteralPath $jsonPath -Force -ErrorAction SilentlyContinue

    $sessionProfile = Get-AnsibleSessionUserProfile -Credential $Credential
    if ($sessionProfile -and ($sessionProfile -ne $ProfilePath)) {
        Write-Warn "ansible session USERPROFILE is '$sessionProfile' (expected '$ProfilePath')."
        Write-Warn 'WSL may be registered under a temporary profile; export will still be attempted.'
    } elseif ($sessionProfile) {
        Write-Info "ansible session USERPROFILE: $sessionProfile"
    }
...
function Move-AnsibleWslDistroBasePath {
    ...
    $set = Invoke-HomelabLibScriptAsUser -Credential $Credential `
        -LibScriptName 'set-ansible-wsl-basepath.ps1' `
        -ScriptArguments @('-DistroName', $DistroName, '-BasePath', $newBasePath)
...
function Invoke-HomelabLibScriptAsUser {
    param(
        [System.Management.Automation.PSCredential]$Credential,
        [string]$LibScriptName,
        [string[]]$ScriptArguments
    )
    Initialize-BootstrapSharedTemp
    $source = Join-Path $global:HomelabWindowsScriptRoot "lib\$LibScriptName"
    if (-not (Test-Path -LiteralPath $source)) {
        throw "Lib script not found: $source"
    }
    $dest = Join-Path $global:BootstrapSharedTemp $LibScriptName
    Copy-Item -LiteralPath $source -Destination $dest -Force
    $psArgs = @(
        '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass',
        '-File', $dest
    ) + @($ScriptArguments)
    return Invoke-AsUser -Credential $Credential -FilePath 'powershell.exe' -ArgumentList $psArgs
}
...
function Get-AnsibleSessionWslDistroBasePath {
    ...
    $run = Invoke-HomelabLibScriptAsUser -Credential $Credential `
        -LibScriptName 'get-ansible-wsl-basepath.ps1' `
        -ScriptArguments @('-DistroName', $DistroName)
    if ($run.ExitCode -ne 0) { return $null }
    return ($run.OutputText -replace [char]0, '').Trim()
}

function Unregister-WslDistroForUser {
    param(
        [System.Management.Automation.PSCredential]$Credential,
        [string]$DistroName
    )
    $result = Invoke-WslCliAsUser -Arguments @('--unregister', $DistroName) -Credential $Credential
    return $result
}
